...
class CAE(nn.Module):
    ...
    def __init__(self):
        ...
        self.d_up_conv_1 = nn.Sequential(
            nn.Conv2d(in_channels=32, out_channels=64, kernel_size=(3, 3), stride=(1, 1)),     # 改(3,3)->(2,2)
            nn.LeakyReLU(),
        ...
        self.d_up_conv_2 = nn.Sequential(
            nn.Conv2d(in_channels=128, out_channels=256, kernel_size=(3, 3), stride=(1, 1)),   # 改(3,3)->(2,2)
            nn.LeakyReLU(),

            nn.ZeroPad2d((1, 1, 1, 1)),             # 改(1, 1, 1, 1)->(1, 0, 1, 0)
            nn.ConvTranspose2d(in_channels=256, out_channels=64, kernel_size=(2, 2), stride=(2, 2)),
            nn.LeakyReLU(),
        )

        # 3x128x128
        self.d_up_conv_3 = nn.Sequential(
            nn.Conv2d(in_channels=64, out_channels=12, kernel_size=(3, 3), stride=(1, 1)),      # 改(3,3)->(2,2)
            nn.LeakyReLU(),

            nn.ReflectionPad2d((2, 2, 2, 2)),            # 改(1, 1, 1, 1)->(1, 0, 1, 0)
            # nn.ConvTranspose2d(in_channels=12, out_channels=3, kernel_size=(2, 2), stride=(2, 2)),
            nn.Conv2d(in_channels=12, out_channels=3, kernel_size=(3, 3), stride=(1, 1)),
            nn.Tanh()
        )
